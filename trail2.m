...
while (iter<50 && ~convergence)
    ...
    for i = 1:5
        X = quantizedObs(gesture).idx{i,1};
        T = size(X,1);
        ...
        zeta{iter,i} = getZeta(A{iter,i}, B{iter,i}, alpha_hat{iter,i},...
                        beta_hat{iter,i}, c{iter,i},X);
        % M STEP
        
        pi_est{iter,i} = gamma{iter,i}(1,:);
        A_est{iter,i} = estA(zeta{iter,i});
        B_est{iter,i} = estB(gamma{iter,i},X);
